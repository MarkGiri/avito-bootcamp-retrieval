...
При проверке на calibration.f частые статьи часто занимали почти весь top-10, отрицания терялись при очистке, а длинные статьи плохо сопоставлялись с короткими запросами. Также статьи, которых не было среди меток обучающего фолда, получали слишком низкий score.

Поэтому был добавлен частотный штраф, сохранены важные смысловые слова, статьи разбиты на фрагменты и добавлен отдельный retrieval-канал по всему корпусу.